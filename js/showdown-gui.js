...
wp.editPost(blog_id, post_id, content);
...
/**
 * Don't modify mimic.js
 * 
 * Proxy to info.webtoolkit.Base64 class
 */
function Base64(value) {
	this.bytes = value;
};
Base64.prototype.encode = function() {
	return info.webtoolkit.Base64.encode(this.bytes);
};
Base64.prototype.decode = function() {
	return info.webtoolkit.Base64.decode(this.bytes);
};
...
//
function onNewPost(){
	if(runtime.canPublish == true && runtime.isPublished == false){
		data = {
				post_type: 'post',
				post_status : 'publish',
				post_title: document.getElementById("postTitle").value,
				post_content: new Base64(inputPane.value)
		};
		postId = wp.newPost(1, data);
		
		if(parseInt(postId, 10) >= 1){
		    $('<input/>').attr({id:"postId",type:"hidden", value:postId}).appendTo("#footer");
		    runtime.isPublished = true;
		    $("#newPost").unbind().attr({id:"editPost"}).bind('mouseup', function(){
		        var isEdited = wp.editPost(1, postId, {
	                post_type: 'post',
	                post_status : 'publish',
	                post_title: document.getElementById("postTitle").value,
	                post_content: new Base64(inputPane.value)
...
function setPaneHeights() {
	...
	// resize all panes
	inputPane.style.height = parseInt(roomLeft, 10) - 20 + "px";
	previewPane.style.height = parseInt(roomLeft, 10) + 30 + "px";
